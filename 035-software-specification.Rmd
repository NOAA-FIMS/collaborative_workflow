...
$$R^*_t=R_t\mathrm{exp}(r_{dev,t}-b_y\frac{\sigma_R^2}{2})$$
...
## Logistic function for maturity and selectivity 
$$x_i=\frac{1}{1+\mathrm{exp}(-ln(19)(i-\nu_1)/\nu_2)}$$
Where xi is the quantity of interest (proportion mature, selected, etc.), i is the index (can be age or size or any other quantity), 1 is the index of 50% mature and 2 is the difference in the index at 50% and the index at 95% (2 = ln(19)/slope parameter from an alternative parameterization). See Appendix A on more details of the parameterization. The selectivity.hpp and maturity.hpp files should right now only include interfaces to the logistic function implemented in common. [from Li et al:
 
## Baranov catch equation 
$$ C_{a,t}=\frac{Fa,t}{F_{a,t}+M}(1-\mathrm{exp}(-(F_{a,t}+M)))N_{a,t}$$
Where Ca,t is the catch at age at time t, F is instantaneous fishing mortality, M is assumed constant over ages and time in the minimum viable assessment model, Na,t is the number of age a fish at time t. 
...
where $B_t$ is total biomass in time $t$, $N$ is total numbers, $w_a$ is weight-at-age $a$.

$$N_t=\sum_{a=0}^AN_{a,t}$$
$$I_t=q\sum_{a=0}^ANt$$
Where q is survey catchability, s is selectivity indexed over the fish’s age selectivity-at-age is constant over time.


$$F_{a,t}=\sum_{a=0}^A s_{a,f}F$$

Catch is in metric tons and survey is in number, so calculating catch weight (CWt) is done as follows:
...
Age composition likelihood links proportions at age from data to model using a multinomial likelihood function. The multinomial and lognormal distributions, including atomic functions are provided within TMB now so we can use it. 
