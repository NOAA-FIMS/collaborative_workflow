...
Run `cmake --build build` and review generated doxygen HTMLS locally 
to ensure they is error-free.
...
and make sure the C++ tests pass before pushing tests to remote feature branch.
 If there are failing tests, run `ctest --test-dir --rerun-failed --output-on-failure` to re-run the failed tests verbosely.
* Ensuring the module runs through the FIMS Github actions on multiple operating systems.
